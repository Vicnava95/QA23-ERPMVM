...
@section('content')
    <div class="container-fluid" style="margin-top: 20px;">
        <div class="row">
            ...
            <div class="col text-center">
                {{-- <a href="#" onclick="location.href='/reportsThisMonth/'+startMonth+'/'+endtMonth+'/'+6;return false;"><div class="btn btn btn-outline-primary btn-sm" >Go to Report</div></a> --}}
                {{-- <h4><a href="#" onclick="location.href='/dashboard/public/reportsThisMonth/'+startMonth+'/'+endtMonth+'/'+6;return false;" class="badge badgeERPButton" data-toggle="tooltip" data-placement="bottom" title="Go To Report"><i class="uil uil-file-bookmark-alt"></i></a></h4> --}}
                <h4><a href="#" onclick="location.href='/reportsThisMonth/'+startMonth+'/'+endtMonth+'/'+6;return false;" class="badge badgeERPButton" data-toggle="tooltip" data-placement="bottom" title="Go To Report"><i class="uil uil-file-bookmark-alt"></i></a></h4>
            </div>
            <div class="col text-center">
...
<div class="container-fluid" > 
    <figure class="text-center">
        <blockquote class="blockquote" style="padding:15px;">
            <h3 class="m-0 font-weight-bold text-dark" id="showDate">Report of {{$startDate}} - {{$endDate}} </h3>
        </blockquote>
    </figure>
    <!-- START - Datepicker -->
      ...
      <!-- START - Secciones por fecha -->
        ...
        <div class="card-body">
                        ...
                        <table class="table table-striped" border="1" > 
                            ...
                            <tbody>
                                @foreach ($allCategorie as $allCate)
                                    @foreach ($totalCategory as $montoCate)
                                        @if ($allCate->id == $montoCate['category'])
                                            <tr>
                            ...
                            <tfoot>
                                <th class="text-center" style="border-color: black;">Total  of {{$startDate}} - {{$endDate}}</th>
                                <th class="text-center" style="border-color: black;"></th>
                                <th class="text-center" style="border-color: black;">${{number_format($total,2)}}</th>
                            </tfoot>
                        </table>
                    </div>
                    <br>
            ...
            <table class="table table-striped" border="1">
                <thead>
                    <tr>
                        <th class="text-center"  style="border-color: black;">Weeks</th>
                ...
                @if ($total != 0)
                <tbody>
                    @foreach ($fechasXsemana as $fechas)
                    <tr>
                        <td style="border-color: black; vertical-align:middle"  width="450" align="center">
                            @switch($fechas['week'])
                                @case($fechas['inicio'])
                                    @if ($fechas['startDateQuery'] == $fechas['week_end'])
                                        <strong>{{$fechas['startDateQuery']}}</strong>
                                    @else
                                        <strong>{{$fechas['startDateQuery']}} - {{$fechas['week_end']}}</strong>
                                    @endif
                                    
                                    @break
                                @case($fechas['final'])
                                    @if ($fechas['week_start'] == $fechas['endDateQuery'])
                                        <strong>{{$fechas['week_start']}}</strong>
                                    @else
                                        <strong>{{$fechas['week_start']}} - {{$fechas['endDateQuery']}}</strong>
                                    @endif
                                    
                                    @break
                                @default
                                <strong>{{$fechas['week_start']}} - {{$fechas['week_end']}}</strong>
                        ...
                        <td style="border-color: black; vertical-align:middle"  width="450" align="center">
                            @for ($i = 0; $i < $cantidadArray; $i++)
                                @foreach ($array[$i] as $a)
                                    @if ($a['day'] >= $fechas['week_start']   && $a['day'] <= $fechas['week_end'])
                                        <div class="row-sm">
                        ...
                        <td style="border-color: black; vertical-align:middle" width="450"align="center">
                           @foreach ($totales as $t)
                                @if($t['weekStart'] ==$fechas['week_start'])
                                    <div class="row-sm">
